...
const baseButtonStyle =
  "w-[164px] h-[52px] flex items-center justify-center text-base font-bold rounded-3xl border-2 border-gray-900 shadow-button";
...
const Button = ({ type, variant, children, disabled }: ButtonProps) => {
  ...
  return (
    <button
      type={type}
      disabled={disabled}
      className={`${baseButtonStyle} bg-gray-200 text-gray-900`}
    >
      {children}
    </button>
  );
