...
const initialValidationStates = {  //앞으로 해당 인풋들 다 여기에 한번에 저장 키:밸류로
  values: {},
  errors: {},
  errorMessages: {},
  isValid: {},
...
function validationReducer(state, action) { //state는 validationStates
  switch (action.type) {
    case ("set_validationState"): {
      const { name, value } = action.payload;
      const validator = validRuleObj[name];
      const isValid = Boolean(validator.isValid(value, state.values["user-password"]))
      const errMsg = isValid ? "" : validator.getErrorMessage(value);

      return ({ //여기서 새로 만들어서 set해주기
...
export function useValidate() {
  const [validationStates, dispatch] = useReducer(validationReducer, initialValidationStates)
