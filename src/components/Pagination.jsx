...
function Pagination() {
  ...
  return (
      <ul aria-label="페이지 네비게이션" className={styles[`items__all-pagination`]}>
        <li aria-label="이전 페이지" className={styles.pagination__prev} onClick={handlePrev}>
          {"‹"}
        </li>
        {pageList.map((i) => (
          <li
        ...
        <li aria-label="다음 페이지" className={styles.pagination__next} onClick={handleNext}>
          {"›"}
        </li>
      </ul>
  );
